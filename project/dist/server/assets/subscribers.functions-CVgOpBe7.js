import { T as TSS_SERVER_FUNCTION, j as createServerFn } from "./server-C1jvFsDS.js";
...
const subscribeEmail_createServerFn_handler = createServerRpc({
  id: "9aeb2a0838b5c4f618356f26ce7663cf7ad69c8fc86148f8280b7452144301d6",
  name: "subscribeEmail",
  filename: "src/lib/subscribers.functions.ts"
}, (opts) => subscribeEmail.__executeServer(opts));
const subscribeEmail = createServerFn({
  method: "POST"
}).inputValidator((input) => schema.parse(input)).handler(subscribeEmail_createServerFn_handler, async ({
  data
}) => {
  const {
    error
...
export {
  subscribeEmail_createServerFn_handler
};
